...
function carregarUsuariosInativos() {
	$.ajax({type:'GET', dataType:'json', url:'admin/getusuariosinativos', timeout:10000,
		success: function(dados){
			if((dados) == "") {
				tabela = "NENHUM USUÁRIO PENDENTE";
			} else {
				tabela = '	<thead>'
	                			+'<tr>'
	            					+'<th>USUÁRIO</th>'
				...
				$.each(dados, function(i, obj) {
					...
					if(!obj.id_facebook) loginMsg = 'Login: '+obj.login;
					else loginMsg = 'Carregando Usuário...';

					tabela += '<tbody r>'
	                            +'<tr id="'+obj.id_facebook+'">'
	                            +'<td><a id="a'+obj.id+'" href="" target="_blank">'+loginMsg+'</a> '+user_nome+'</td>'
	                            +'<td><a href="admin/ativarusuariofinal?id='+obj.id+'">Ativar Usuário Final</a> | <a href="admin/ativarlojista?id='+obj.id+'">Ativar Lojista</a> | <a href="admin/ativarshopping?id='+obj.id+'">Ativar Shopping</a> | ( <a href="transacoes?id_usuario='+obj.id+'">Transações</a> )</td>'
	                            +'</tr>'
	                        +'</tbody>';
			...
			$('#tableUsuariosInativos').html(tabela);
		},
		error: function(){
...
function carregarUsuariosAtivos() {
	...
	$.ajax({type:'GET', dataType:'json', url:'admin/getusuariosativos', timeout:10000,
		success: function(dados){
			exibirUsuariosAtivos(titulo,dados);
		},
		error: function(){
			console.warn("Erro ao carregar os "+titulo);
		}
	});
}

function carregarUsuariosFinaisAtivos() {
	titulo = "Usuários Finais Ativos";
	$.ajax({type:'GET', dataType:'json', url:'admin/getusuariosfinaisativos', timeout:10000,
		success: function(dados){
			tipoativo = "final";
			exibirUsuariosAtivos(titulo,dados);
		},
...
function carregarLojistasAtivos() {
	titulo = "Lojistas Ativos";
	$.ajax({type:'GET', dataType:'json', url:'admin/getlojistasativos', timeout:10000,
		success: function(dados){
			tipoativo = "lojista";
			exibirUsuariosAtivos(titulo,dados);
		},
...
function carregarAdminsAtivos() {
	titulo = "Administradores Ativos";
	$.ajax({type:'GET', dataType:'json', url:'admin/getadminsativos', timeout:10000,
		success: function(dados){
			tipoativo = "admin";
			exibirUsuariosAtivos(titulo,dados);
		},
...
function carregarShoppingsAtivos() {
	titulo = "Shoppings Ativos";
	$.ajax({type:'GET', dataType:'json', url:'admin/getshoppingsativos', timeout:10000,
		success: function(dados){
			tipoativo = "shopping";
			exibirUsuariosAtivos(titulo,dados);
		},
...
function carregarPreCadastros(tipo) {
	if(tipo == 4) {
		titulo = "Shoppings pré-cadastrados";
		tipoativo_pre = "shopping";
		url = "admin/getallshoppingsativos";
	}
	if(tipo == 2) {
		titulo = "Lojas pré-cadastradas";
		tipoativo_pre = "lojista";
		url = "admin/getalllojistasativos";
	}
	
	$('#precadastro_titulo').html(titulo);
	$('#lista_precadastros').html('');
	
	$.ajax({type:'GET', dataType:'json', url:url, timeout:10000,
		success: function(dados){
			exibirPreCadastros(titulo,dados);
		},
		error: function(){
			console.warn("Erro ao carregar os "+titulo);
		}
...
function carregarLojistasByShopping(id_shopping, nome_shopping) {
	titulo = "Lojas pré-cadastradas do "+nome_shopping;
	tipoativo_pre = "lojista";
	url = "admin/getlojistasbyshopping";
	
	$('#precadastro_titulo').html(titulo);
	$('#lista_precadastros').html('');
	
	$.ajax({data: {id_shopping:id_shopping}, type:'GET', dataType:'json', url:url, timeout:10000,
		success: function(dados){
			exibirPreCadastros(titulo,dados);
		},
		error: function(){
...
function carregarCidades() {
	cidades = new Array();
	$.ajax({type:'GET', dataType:'json', url:'sistema/getcidadescadastradas', timeout:10000,
		success: function(dados){
			$.each(dados, function(i, obj) {
				cidades.push(obj);
			});
...
function carregarEstados() {
	estados = new Array();
	$.ajax({type:'GET', dataType:'json', url:'sistema/getestadoscomcidade', timeout:10000,
		success: function(dados){
			$.each(dados, function(i, obj) {
				estados.push(obj);
			});
...
var estadosCadastrados;
function carregarEstadosCadastrados() {
	estadosCadastrados = new Array();
	$.ajax({type:'GET', dataType:'json', url:'sistema/getestadoscadastrados', timeout:10000,
		success: function(dados){
			$.each(dados, function(i, obj) {
				estadosCadastrados.push(obj);
			});
			listarEstadosCadastrados();
		},
		error: function(){
			console.warn("Erro ao carregar os estados cadastrados");
		}
	});
}

function listarEstadosCadastrados() {
	lista = '<select name="id_estado2" id="id_estado2" onclick="listarTodasCidades();">';
	lista += '<option>Selecione um estado</option>';
	$.each(estadosCadastrados, function(i, obj) {
		lista += '<option value='+obj.id+'>'+obj.nome+'</option>';
	});
	lista += '</select>';
	$('#listaEstados2').html(lista);
}
...
function listarTodasCidades() {
	cidades_selecionadas = todasCidades[$('#id_estado2').prop("selectedIndex")];
	
	lista = '<select name="id_cidade2" id="id_cidade2">';
	lista += '<option>Selecione uma cidade</option>';
	$.each(cidades_selecionadas, function(i, obj) {
		lista += '<option value="'+obj+'">'+obj+'</option>';
	});
	lista += '</select>';
	$('#listaCidades2').html(lista);
}

function listarEstados() {
	lista = '<select name="id_estado" id="id_estado" onclick="listarCidades();">';
	lista += '<option>Selecione um estado</option>';
	$.each(estados, function(i, obj) {
		lista += '<option value='+obj.id+'>'+obj.nome+'</option>';
	});
	lista += '</select>';
	$('#listaEstados').html(lista);
}

function listarCidades() {
	cidades_selecionadas = new Array();
	$.each(cidades, function(i, obj) {
		if(obj.id_estado == $('#id_estado').val())
			cidades_selecionadas.push(obj);
	});
	
	lista = '<select name="id_cidade" id="id_cidade">';
	lista += '<option>Selecione uma cidade</option>';
	$.each(cidades_selecionadas, function(i, obj) {
		lista += '<option value='+obj.id+'>'+obj.nome+'</option>';
	});
	lista += '</select>';
	$('#listaCidades').html(lista);
}

function listarShoppings(id_lojista) {
	titulo = "Shoppings Ativos";
	$.ajax({type:'GET', dataType:'json', url:'admin/getshoppingsativos', timeout:10000,
		success: function(dados){
			lista = "";
			$.each(dados, function(i, obj) {
				lista += '<p><a id="a'+obj.id+'" onclick="event.preventDefault();alterarShopping('+id_lojista+','+obj.id+')" href="">Carregando Shopping...</a></p>';
			...
			lista += '<p><a onclick="event.preventDefault();alterarShopping('+id_lojista+',0)" href="">FICAR SEM SHOPPING</a></p>';
			$('#listaShoppings').html(lista);
		},
		error: function(){
			console.warn("Erro ao listar os shoppings");
		}
	});
}

function listarShoppingsCadastrados() {
	$.ajax({type:'GET', dataType:'json', url:'admin/getallshoppingsativos', timeout:10000,
		success: function(dados){
			lista = '<select name="id_shopping" id="id_shopping">';
			$.each(dados, function(i, obj) {
				lista += '<option value="'+obj.estabelecimento.id+'">'+obj.estabelecimento.nome_fantasia+'</option>';
			});
			lista += '</select>';
			$('#listaShoppingsCadastrados').html(lista);
		},
		error: function(){
			console.warn("Erro ao listar os shoppings");
		}
...
function listarEstabelecimentosCategorias() {
	$.ajax({type:'GET', dataType:'json', url:'admin/getestabelecimentoscategorias', timeout:10000,
		success: function(dados){
			lista = '<select name="categoria" id="categoria">';
			$.each(dados, function(i, obj) {
				lista += '<option value="'+obj.id+'">'+obj.nome+'</option>';
			});
			lista += '</select>';
		...
		error: function(){
			console.warn("Erro ao listar as categorias");
		}
	});
}

function alterarShopping(id_lojista, id_shopping) {
	titulo = "Shoppings Ativos";
	$.ajax({data: {id_lojista:id_lojista, id_shopping:id_shopping}, type:'GET', dataType:'json', url:'admin/setshopping', timeout:10000,
		success: function(dados){
			if(dados[0]) {
				carregarLojistasAtivos();
				$('#alterarSh-modal').modal('hide');
			} else {
				console.debug("O Shopping não foi alterado");
			}
		},
		error: function(){
			console.warn("Erro ao alterar o shopping do lojista");
		}
	});
}

function exibirUsuariosAtivos(titulo,dados) {
	tabela = '	<thead>'
		+'<tr>'
			+'<th>USUÁRIO</th>';
	if(tipoativo == "lojista") tabela += '<th>SHOPPING</th>';
	tabela +='<th colspan="2">CRÉDITOS</th>'
			+'<th>AÇÃO</th>'
			+'</tr>'
	+'</thead>';
	...
	$.each(dados, function(i, obj) {
		...
		if(obj.nome_fantasia) user_nome = "("+obj.nome_fantasia+")";
		
		if(!obj.id_facebook) {
			if(obj.login)
				loginMsg = 'Login: '+obj.login;
			else
				loginMsg = "Pré-cadastro ";
		} else loginMsg = 'Carregando Usuário...';
		
		tabela += '<tbody r>'
				+'<tr id="'+obj.id_facebook+'">'
		            +'<td><a id="a'+obj.id+'" href="" target="_blank">'+loginMsg+'</a> '+user_nome+'</td>';
		            if(tipoativo == "lojista") {
		            	tabela += 	'<td>';
		            	if(obj.id_shopping) {
		            					tabela += '<button data-toggle="modal" href="#alterarSh-modal" onclick="listarShoppings('+obj.id+')">Alterar Shopping</button> ';
		            					tabela += '<a id="sh'+obj.id+'" href="" target="_blank">'+obj.id_shopping+'</a>';
		            	} else {
		            					tabela += '<button data-toggle="modal" href="#alterarSh-modal" onclick="listarShoppings('+obj.id+')">Definir Shopping</button>';
		            	}
		            	tabela +=	'</td>';
		            }
		tabela +=	'<td>'+obj.creditos+'</td>'
		            ...
		            +'<td><a href="transacoes?id_usuario='+obj.id+'">Transações</a>'; 
		if((obj.login) && (obj.login != "")) tabela += ' | <a href="#" onclick="event.preventDefault();confirmResetarSenha('+obj.id+');">Resetar Senha</a> ';
		tabela +=   ' | <a href="admin/desativarusuario?id='+obj.id+'">Desativar Usuário</a>';
		if((tipoativo == 'lojista') || (tipoativo == 'shopping')) tabela += '<br><a target="_blank" href="https://maps.google.com.br/maps?q='+obj.latitude+',+'+obj.longitude+'&hl=pt-BR&t=h&z=18">Ver No Mapa</a>'; 
		tabela +=   '</td>'
		        +'</tr>'
		    +'</tbody>';
		...
		FB.api('/'+obj.id_shopping, function(response) {
			$('#sh'+obj.id).html(response.name);
			$('#sh'+obj.id).attr('href',response.link);
		});
	
	});
	tabela += '</table>';
	$('#h2_usuariosAtivos').html(titulo);
	$('#tableUsuariosAtivos').html(tabela);
}

function exibirPreCadastros(titulo,dados) {
	tabela = '	<thead>'
		+'<tr>'
			+'<th>ESTABELECIMENTO</th>';
	if(tipoativo_pre == "lojista") tabela += '<th>SHOPPING</th>';
	else tabela += '<th>LOJISTAS</th>';
	tabela +='<th>AÇÃO</th>'
			+'</tr>'
	+'</thead>';

	$.each(dados, function(i, obj) {
		if(!obj.estabelecimento.nome_fantasia) user_nome = "";
		else user_nome = obj.estabelecimento.nome_fantasia;

		
		tabela += '<tbody r>'
				+'<tr id="'+obj.estabelecimento.id+'">'
		            +'<td><b style="cursor:pointer;" onclick="$(\'#modalinfo'+obj.estabelecimento.id+'\').toggle()">'+user_nome+'</b>'
		            +'	    <div id="modalinfo'+obj.estabelecimento.id+'" style="display:none;">'
		            +'	        <div>Categoria: '+obj.categoria+'</div>'
					+'	        <div>Endereço: '+obj.estabelecimento.endereco+'</div>'
					+'	        <div>Número: '+obj.estabelecimento.nr+'</div>'
					+'	        <div>Complemento: '+obj.estabelecimento.complemento+'</div>';
		if(obj.telefone != "") tabela += '<div>Telefone: '+obj.telefone[0].numero+'</div>';
		tabela += '		</div>'
		            +'</td>';
		            if(tipoativo_pre == "lojista") {
		            	tabela += 	'<td>';
		            	if(obj.estabelecimento.id_shopping) {
//		            					tabela += '<button data-toggle="modal" href="#alterarSh-modal" onclick="listarShoppings('+obj.estabelecimento.id+')">Alterar Shopping</button> ';
		            					tabela += obj.nome_shopping;
		            	} else {
//		            					tabela += '<button data-toggle="modal" href="#alterarSh-modal" onclick="listarShoppings('+obj.estabelecimento.id+')">Definir Shopping</button>';
		            	}
		            	tabela +=	'</td>';
		            } else {
		            	tabela += '<td>'+obj.qtd_lojistas+'</td>';
		            }
		tabela +=   '<td><button onclick="removerPreCadastro('+obj.estabelecimento.id+')">Remover Pré-Cadastro</span></button>';
		if((tipoativo_pre == "shopping") && (obj.qtd_lojistas > 0)) tabela += ' <button onclick="carregarLojistasByShopping('+obj.estabelecimento.id+',\''+user_nome+'\')">Ver Lojas</button>';
		tabela +=   '</td>'
		        +'</tr>'
		    +'</tbody>';
	...
	$('#precadastro_titulo').html(titulo);
	$('#lista_precadastros').html(tabela);
}
...
function atualizarUsuarios() {
	...
	case 'admin':
		carregarAdminsAtivos();
		break;
		
	case 'shopping':
		carregarShoppingsAtivos();
		break;
...
function confirmResetarSenha(id_usuario) {
	html = ''
		+'<div id="msgConfirm" title="Resetar Senha">'
		+'<p>Deseja realmente resetar a senha deste usuário?</p>'
		+'</div>';
$('body').append(html);
$( "#msgConfirm" ).dialog({ buttons: { "Confirmar": function() { $(this).dialog("close");resetarSenha(id_usuario); }, "Cancelar": function() { $(this).dialog("close"); } } });
}

function resetarSenha(id_usuario) {
	$.ajax({data: {id_usuario:id_usuario}, type:'GET', dataType:'json', url:'admin/resetarsenha', timeout:10000,
		success: function(dados){
			$.alert('',dados[0].mensagem);
			
		},
		error: function(){
			alert('Erro ao enviar pedido de resetar senha.');
			console.warn("Erro ao enviar pedido de resetar senha..");
		}
	});
}

function salvarCidade() {
	id_estado = $('#id_estado2 option:selected').val();
	cidade = $('#id_cidade2 option:selected').val();
	
	statusMsg = $('#modalcadastrocidades #statusMsg');
	statusMsg.fadeOut();
	if(nome_fantasia != "") {
		$('#modalcadastrocidades input').each(function(){ $(this).attr("disabled",true); });
		$('#modalcadastrocidades button').each(function(){ $(this).attr("disabled",true); });
		$.ajax({data: {id_estado:id_estado, cidade:cidade},type:'POST', dataType:'json', url:'admin/cadastrocidade', timeout:10000,
			success: function(dados){
				if(dados[0].sucesso) {
				...
				$('#modalcadastrocidades input').each(function(){ $(this).attr("disabled",false); });
				$('#modalcadastrocidades button').each(function(){ $(this).attr("disabled",false); });
				statusMsg.html(dados[0].mensagem);
				statusMsg.fadeIn();
			},
			error: function(){
				console.warn("Erro ao cadastrar cidade");
			}
		});
	} else {
		statusMsg.html('Um ou mais campos obrigatórios não foram preenchidos');
		statusMsg.removeClass('label-success');
		statusMsg.addClass('label-important');
		statusMsg.fadeIn();
	}
...
function salvarPreCadastro() {
	...
	id_shopping = $('#id_shopping option:selected').val();
	categoria = $('#categoria option:selected').val();
	
	statusMsg = $('#modalprecadastro #statusMsg');
	statusMsg.fadeOut();
	if(nome_fantasia != "") {
		$('#modalprecadastro input').each(function(){ $(this).attr("disabled",true); });
		$('#modalprecadastro button').each(function(){ $(this).attr("disabled",true); });
		$.ajax({data: {nome_fantasia:nome_fantasia, endereco:endereco, nr:nr, complemento:complemento, telefone:telefone, tipo:tipo, id_shopping:id_shopping, categoria:categoria},type:'POST', dataType:'json', url:'admin/precadastro', timeout:10000,
			success: function(dados){
				if(dados[0].sucesso) {
//					listarShoppingsCadastrados();
					statusMsg.removeClass('label-important');
					statusMsg.addClass('label-success');
				...
				$('#modalprecadastro input').each(function(){ $(this).attr("disabled",false); });
				$('#modalprecadastro button').each(function(){ $(this).attr("disabled",false); });
				statusMsg.html(dados[0].mensagem);
				statusMsg.fadeIn();
			},
			error: function(){
				console.warn("Erro ao pré-cadastrar estabelecimento");
			}
		});
	} else {
		statusMsg.html('Um ou mais campos obrigatórios não foram preenchidos');
...
function removerPreCadastro(id) {
	statusMsg = $('#modalexibirprecadastros #statusMsg');
	statusMsg.fadeOut();
	if(nome_fantasia != "") {
		$('#modalcompletarcadastro input').each(function(){ $(this).attr("disabled",true); });
		$.ajax({data: {id:id},type:'GET', dataType:'json', url:'admin/removerprecadastro', timeout:10000,
			success: function(dados){
				if(dados[0].sucesso) {
					listarShoppingsCadastrados();
					statusMsg.removeClass('label-important');
				...
				if(tipoativo_pre == "shopping")
					carregarPreCadastros(4);
				else
					carregarPreCadastros(2);
